...
namespace Theodore {
  ...
  bool WindowsPlatform::CreatePlatformWindows(const std::string& title, int width, int height, bool fullscreen, int majorVersion, int minorVersion, int multisample, WindowStyle style,
    ...
    /*
      ...
      CreateWindow -> GetDC -> ChoosePixelFormat -> SetPixelFormat -> wglCreateContext ->
    ...
    */
    ...
    if (majorVersion != 0 || minorVersion != 0) {
      ...
      if (platform->mIsMultisampleSupported) {
        // with multisampling
        const int pixelAttribs[] = {WGL_SAMPLES_ARB,
                                    multisample,
  ...
  void Platform::SwapBuffer() { SwapBuffers(WindowsPlatform::instance->mHdc); }
