...
namespace Theodore {
  FontRenderer::FontRenderer() : Renderer("FontRenderer"), font(nullptr), color(Color()), text(std::string()) {
		primitive = Primitive::Triangles;
		pipeline = Shader::Find("Font");
  }

  FontRenderer::FontRenderer(Font* font) : Renderer("FontRenderer"), font(nullptr), color(Color()), text(std::string()) {
		pipeline = Shader::Find("Font");
    SetFont(font);
  }
  ...
  Font* FontRenderer::GetFont() const { return font; }

  Color FontRenderer::GetColor() const { return color; }

  std::string FontRenderer::GetText() const { return text; }
  ...
  void FontRenderer::Render() {
    pipeline->Use();
    pipeline->SetUniform("textColor", color);
    pipeline->SetUniform("projection", Matrix4x4::Orthogonal(0.0f, 800.0f, 0.0f, 600.0f, 0.1f, 1000.f));
    ...
    scale.z = 0.1;
    for (auto& ch : text) {
      GlyphInfo* glyph = font->FindGlyph(ch);
      if (glyph) {
        //        glActiveTexture(GL_TEXTURE0);
