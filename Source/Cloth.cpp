...
namespace Theodore {
  ...
  Cloth::Cloth(int width, int height)
      : mEnabled(true), mUseGravity(true), mClothSolverFrequency(15), mWidth(width),
        mHeight(height), mTransform(), mVao(), mBuffer(Enumeration::BufferVertex), mProgram() {
  ...
  float Cloth::GetDamping() { return Cloth::Damping; }

  Particle* Cloth::GetParticle(int x, int y) { return &mParticles[y * mWidth + x]; }
  ...
  void Cloth::addWindForcesForTriangle(Particle* p1, Particle* p2, Particle* p3,
  ...
  void Cloth::WindForce(const Vector3d& direction) {
    for (int x = 0; x < mWidth - 1; x++) {
      for (int y = 0; y < mHeight - 1; y++) {
        addWindForcesForTriangle(GetParticle(x + 1, y), GetParticle(x, y), GetParticle(x, y + 1),
                                 direction);
        addWindForcesForTriangle(GetParticle(x + 1, y + 1), GetParticle(x + 1, y),
  ...
  void Cloth::Render(const Camera& cam) {
    ...
    mProgram.SetUniform(
        mProgram.GetUniform("color"),
        Color::Lerp(Color::purple, Color::green, Math::PingPong(Time::ElapsedTime() / 2.f, 1.f)));
    ...
    // Graphics::DrawArraysInstanced(mVao, Enumeration::Primitive::Triangles, 0, 6, mWidth *
    // mHeight);
  }
