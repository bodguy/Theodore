...
namespace Theodore {
	...
	KeyCode InputHandler::GetPositive() const {
		return mPositive; 
	}
	
	KeyCode InputHandler::GetNegative() const {
		return mNegative; 
	}

	float InputHandler::GetAccumulator() const {
		return mAccumulator; 
	}
	...
	void InputHandler::ResetAccumulator() {
		mAccumulator = 0.f; 
	}
	
	float InputHandler::GetDelta() const {
		return mDeltaSpeed; 
	}
