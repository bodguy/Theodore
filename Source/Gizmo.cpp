...
namespace Quark { 
	Gizmo::Gizmo(Enumeration::GizmoType type) :Component("Gizmo"), mVao(nullptr), mTransform(), mProgram(nullptr), mType(type) {
		Shader* vs = AssetManager::RequestShader("Core/Shaders/gizmo/vs.glsl", Enumeration::VertexShader);
		Shader* fs = AssetManager::RequestShader("Core/Shaders/gizmo/fs.glsl", Enumeration::FragmentShader);
