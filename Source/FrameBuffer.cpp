...
namespace Quark {
	...
	Texture2D* FrameBuffer::GetRenderTexture(Attachment attach) {
	...
	void FrameBuffer::GetBufferInfo() {
		/*
			...
			GL_FRAMEBUFFER_ATTACHMENT_OBJECT_TYPE: returns the type of object which contains either GL_RENDERBUFFER, GL_TEXTURE, GL_NONE.
			...
			GL_FRAMEBUFFER_ATTACHMENT_TEXTURE_CUBE_MAP_FACE: initial value is GL_TEXTURE_CUBE_MAP_POSITIVE_X
		*/
		glBindFramebuffer(GL_FRAMEBUFFER, mFrameBufferID);
